...
public class RoundPainter extends BasePainter {    
    ...
    */
    ...
    protected Rectangle2D updateShape(Annotation aO) {            
        ...
        for (String s : aO.getText()) {
            ...
            if (aO.isMultiline()) {
                ...
                width += b.getWidth();
                height = fm.getMaxCharBounds(iGC).getHeight();// Math.max(height, b.getHeight());
            }
        }
          
        final double scaleX = iView.virtualToScreen().getScaleX();      
        
        Rectangle2D rect = new Rectangle2D.Double(aO.getShape().getBounds2D().getX() * scaleX, 
                                aO.getShape().getBounds2D().getY() * scaleX,                                                                                        
                                width + fm.getMaxCharBounds(iGC).getWidth(), 
                                height + fm.getMaxCharBounds(iGC).getHeight());
    ...
    public void paint(Annotation aO) {            
        ...
        final Rectangle2D temp = updateShape(aO);    
        
        iGC.setColor(aO.getColor());

        final java.awt.FontMetrics fm = iGC.getFontMetrics();
        final Rectangle2D bnds = fm.getMaxCharBounds(iGC);
        
        final double gapX = bnds.getWidth()/2;
        final double gapY = bnds.getHeight()/2;
                
        if (!aO.isMultiline()) {
            String str = new String();
            str = aO.getText().stream().map((s) -> s).reduce(str, String::concat);            
            iGC.drawString(str, (int)(temp.getX() + gapX), (int)(temp.getY() + temp.getHeight() - gapY));                   
        } else {            
            final double stepY = temp.getHeight() / aO.getText().size();
            double posY = temp.getY() + stepY - gapY;
            
            for(String str : aO.getText()) {                               
                iGC.drawString(str, (int)(temp.getX()+gapX), (int)posY);
                posY += stepY;
            }
        }
    ...
    public void paint(Handle aO) {        
        Rectangle2D temp = iView.virtualToScreen().createTransformedShape(aO.getShape()).getBounds2D();                           
        iGC.setColor(Color.RED);       
        iGC.drawRoundRect((int)temp.getX(), (int)temp.getY(), (int)temp.getWidth(), (int)temp.getHeight(), 
                         (int)(temp.getWidth()*ARC_WIDTH), (int)(temp.getHeight()*ARC_HEIGHT));     
    }
